    ...
    for name in files:
        ...
        for c in result:
            ...
            for i in range(int(duration / (int(options.splitsize)*60)) + 1):##??????????
                ...
                os.chdir(options.workdir)
                cmd = "touch ffm/output.log;/usr/local/src/ffmpeg-git-20181015-64bit-static/ffmpeg -y -i "+options.srcD+"/"+name+" -codec: copy -ss "+str(i*int(options.splitsize)*60)+" -t "+_t+" "+options.targetD+"/"+options.prefix+newname+'_'+string.replace(('%'+str(nameLength)+'s')%str(i),' ','0')+"."+options.fileType + ' >& ffm/output.log'
                for i in cmd.split(';'):
                    print i
                    time.sleep(2)
                    t1=subprocess.Popen(i,shell=True)
